...
function allowed_html_custom($allowed, $context){
  ...
  }else if ($context === 'allowed_slider_title_html') {
    global $wdwt_allowed_for_slider_title;
    $allowed = $wdwt_allowed_for_slider_title;
  }
  return $allowed;
}

add_filter('wp_kses_allowed_html','allowed_html_custom',10, 2);
...
function wdwt_param_clean($input, $default = false, $param_type, $sanitize_type = '', $validate_type = '', $valid_options = array())
{
  ...
  $allowed_slider_title_html = apply_filters( 'wp_kses_allowed_html', "", "allowed_slider_title_html" );
  ...
  switch ($param_type) :
    ...
    case 'text':
      ...
      if ( 'sanitize_parallax_html_field' == $sanitize_type  ) {
        $input = wp_kses(stripslashes($input), $allowed_slider_desc_html);
      }
      ...
      if ( 'sanitize_head_html_field' == $sanitize_type  ) {
        $input = wp_kses(stripslashes($input),$allowed_custom_head);
      }
      break;
...
class WDWT_customizer_sanitizer {
  ...
  static function text_sanitize_parallax_html_field($input, $setting){
    $allowed_slider_desc_html = apply_filters( 'wp_kses_allowed_html', "", "allowed_slider_desc_html" );
